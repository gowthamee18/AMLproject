...
#
# WARNING: Building a Dockerfile can take a fair bit of time, therefore
# it can be useful to comment out everything AFTER the TODO you are currently working on!
# Once you have succesfully  built a given layer or Docker instruction (e.g., FROM, LABEL, RUN, ect) 
# then this layer should be cached and run immdiately next time unless a layer or instruction
...
# Use Ubuntu version 18.04 as the base image
# Version 18.04 is not working for Mac M1 Chip; So upgraded to latest version of ubuntu:latest
...
LABEL maintainer="Sri_Gowthami_Sunkavalli"
...
#    to your first name and last name. For example, HOST_USER="john Doe".

# Create an argument for HOST_USER
ARG HOST_USER="Sri_Gowthami_Sunkavalli"
...
# Switch to the created user using USER instruction
USER $HOST_USER
...
# Set working directory using WORKDIR instruction
WORKDIR $HOME
...
#    Given it is not there already, be sure to add the '&&' before the command and 
#    the '\' at the end of the command as this RUN command runs multiple commands at once! 
# 2. Add the 'rm' command to remove the conda_requirements.txt. Given it is not there 
#    already, be sure to add the '&&' before the command to indicate it is part of the
#    chain of commands for RUN! THIS IS OUR LAST COMMAND SO WE DON'T NEED TO ADD
...
RUN  conda config --system --prepend channels conda-forge \
    ...
    ##### TODO 7-1 #####
    && conda install --yes --file $HOME/conda_requirements.txt \
    && conda clean --all -f -y \
    ##### TODO 7-2 #####
    && rm $HOME/conda_requirements.txt
...
#    as small as possible! Given it is not there already, be sure to add the '&&' 
#    before the command and the '\' at the end of the command as this RUN command 
#    runs multiple commands!
# 2. Add the 'rm' command to remove the pip_requirements.txt. Given it is not there 
#    already, be sure to add the '&&' before the command to  indicate it is part
#    of the chain of commands for RUN!
RUN pip install --upgrade pip \
    ##### TODO 8-1 #####
    && pip install --user -r $HOME/pip_requirements.txt --no-cache-dir \
    ##### TODO 8-2 #####
    && rm $HOME/pip_requirements.txt
